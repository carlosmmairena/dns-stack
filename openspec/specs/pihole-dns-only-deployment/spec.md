# pihole-dns-only-deployment Specification

## Purpose
Mantener el despliegue de Pi-hole alineado exactamente con su alcance real — resolución DNS únicamente, sin DHCP, sin Router Advertisements IPv6 y sin servicios NTP — de modo que el contenedor no declare capabilities Linux ni funciones activas que no correspondan a ninguna necesidad del stack, y que el modo de escucha DNS sea el correcto para una topología de red bridge de Docker con puertos publicados hacia la LAN.
