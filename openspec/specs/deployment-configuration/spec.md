...
## Purpose
Separar en `docker-compose.yml` las decisiones de arquitectura del stack (DNSSEC, `listeningMode`, rate limiting) de los hechos específicos de cada instalación (huso horario, subnet LAN), de modo que estos últimos vivan en `.env` en vez de hardcodeados en un archivo trackeado por git, sin perder la forma revisable de las decisiones de política que sí permanecen en el compose.
