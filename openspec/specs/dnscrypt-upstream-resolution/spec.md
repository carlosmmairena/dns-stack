# dnscrypt-upstream-resolution Specification

## Purpose
Eliminar el punto único de disponibilidad/confianza del upstream DNS resolviendo consultas a través de al menos dos resolvers DNSCrypt independientes, cada uno vetted con el mismo nivel de garantías (protocolo DNSCrypt, validación DNSSEC, política de no-log, filtrado de dominios maliciosos), con failover automático entre ellos y sin mezclar protocolos (solo DNSCrypt) para no romper la cadena de confianza ya documentada del stack.
